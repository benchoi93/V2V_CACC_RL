...
class CriticGraphPolicy(nn.Module):
    ...
    def forward(self, state, action, mode='train'):
        ...
        self.x1 = torch.stack(self.x1, dim=1)  # (batch_size, num_limbs, 1)
        self.x2 = torch.stack(self.x2, dim=1)  # (batch_size, num_limbs, 1)

        return self.x1, self.x2

    def Q1(self, state, action, mode="train"):
        x1, _ = self.forward(state, action, mode)
        return x1
    ...
    def bottom_up_transmission(self, node):
        ...
        if self.td:
            ...
            self.msg_up[node], self.x1[node], self.x2[node] = self.sNet[node](state, action, *msg_in)
    ...
    def top_down_transmission(self, node):
        ...
        if self.msg_down[node] is not None:
